...
-- Create function to get friend activity with explicit integer casting
    ...
    SELECT 
      ...
      jsonb_agg(
        jsonb_build_object(
          ...
          'sets', ce.sets::integer,
          'reps', ce.reps::integer,
...
-- Create function to get friend stats with explicit integer casting
...
BEGIN
  ...
  SELECT 
    ...
    COUNT(DISTINCT ce.exercise_id)::integer as total_exercises,
    SUM(ce.sets)::integer as total_sets,
    SUM(ce.weight_lbs * ce.sets * ce.reps)::numeric as total_weight
  FROM profiles p
