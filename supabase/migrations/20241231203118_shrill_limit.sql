/*
  ...
  1. Changes
    - Convert scheduled_date from timestamptz to date
    - Update functions to handle simple dates
    - Add index for date queries
*/

-- Modify user_workouts table to use date instead of timestamptz
...
-- Recreate get_friend_workouts with date type
...
DECLARE
  week_start date := current_date - cast(extract(dow from current_date) as int);
...
-- Recreate get_friend_workout_stats with date type
