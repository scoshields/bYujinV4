/*
  ...
  1. Changes
    - Fix ambiguous 'completed' column references
...
-- Recreate get_friend_workouts with qualified column references
  ...
  WITH workout_exercise_stats AS (
    SELECT 
      ...
      COUNT(DISTINCT CASE WHEN we.id IN (
        SELECT es.workout_exercise_id 
        FROM exercise_sets es
        WHERE es.completed = true
        GROUP BY es.workout_exercise_id
        HAVING COUNT(*) = COUNT(CASE WHEN es.completed THEN 1 END)
...
-- Recreate get_friend_workout_stats with qualified column references
