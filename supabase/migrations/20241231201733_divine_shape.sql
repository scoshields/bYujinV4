/*
  ...
  1. Changes
    ...
    - Fix type mismatch between date and timestamptz
...
DECLARE
  week_start timestamptz := date_trunc('week', CURRENT_TIMESTAMP AT TIME ZONE 'UTC');
...
BEGIN
  ...
  WITH exercise_stats AS (
    ...
    WHERE EXISTS (
      ...
      AND uw.scheduled_date AT TIME ZONE 'UTC' BETWEEN week_start AND week_end
    )
  ),
  workout_stats AS (
    SELECT 
  ...
  SELECT 
    w.id,
    w.name,
    w.scheduled_date AT TIME ZONE 'UTC',
    w.completed,
    p.id as user_id,
    p.username,
    p.first_name,
...
BEGIN
  ...
  WITH exercise_completion AS (
    ...
    WHERE w.scheduled_date AT TIME ZONE 'UTC' BETWEEN p_start_date AND p_end_date
  )
  SELECT 
    p.id,
    p.username,
  ...
  LEFT JOIN user_workouts w ON 
    w.user_id = p.id AND
    w.scheduled_date AT TIME ZONE 'UTC' BETWEEN p_start_date AND p_end_date
