...
-- Recreate get_friend_workouts with proper completion tracking
...
BEGIN
  ...
  WITH exercise_stats AS (
    SELECT 
      ...
      COUNT(CASE WHEN es.completed OR es.weight_lbs IS NOT NULL THEN 1 END) as completed_sets
...
-- Recreate get_friend_workout_stats with proper completion tracking
...
BEGIN
  ...
  SELECT 
    ...
    COUNT(CASE WHEN es.completed OR es.weight_lbs IS NOT NULL THEN 1 END)::integer as completed_sets
  FROM profiles p
