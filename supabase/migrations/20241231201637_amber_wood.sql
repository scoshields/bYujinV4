/*
  ...
  1. Changes
    - Improve exercise counting logic to properly track weekly totals
    - Consider an exercise completed only when all its sets are completed
    - Add proper date filtering for weekly stats
...
-- Recreate get_friend_workouts with improved exercise tracking
...
DECLARE
  week_start timestamptz := date_trunc('week', CURRENT_TIMESTAMP);
  week_end timestamptz := date_trunc('week', CURRENT_TIMESTAMP) + interval '6 days';
BEGIN
  RETURN QUERY
...
-- Recreate get_friend_workout_stats with improved weekly stats
